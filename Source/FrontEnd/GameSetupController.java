...
public class GameSetupController extends StateLoad {
	...
	public void initialize(URL location, ResourceBundle resources) {
		if (selectGameBoard.getValue() == null) {
			...
			String[] customBoards;
			File customBoardLocation = new File("SaveData\\CustomLevels");
			customBoards = customBoardLocation.list();
			if (customBoards != null) {
				for (String customBoard : customBoards) {
					customBoard = customBoard.substring(0, customBoard.length() - 4);
					selectGameBoard.getItems().add(customBoard);
					if (getInitData() != null) {
						getInitData().put("LevelType", "custom");
					}
				}
			}
